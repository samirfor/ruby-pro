...
module SMS
  ...
  def self.enviar(celular, texto)
    ...
    webhost = "http://torpedo.oiloja.com.br"

    begin
        ## Send form by POST
      torpedo = HTTPClient.new webhost
      action = URI.parse "#{webhost}/oitorpedo/EnviaSMSController?acao=envioTorpedoPasso1"
      remetente = Celular.new(85, 88016247)
      destinatario = celular
      form = Hash.new
      form["hidParametro"] = 'LIBERADO'
      form['ddd'] = destinatario.ddd.to_s
      form['numero'] = destinatario.numero.to_s
      form['assinatura'] = "RSOnline"
      form["mensagem"] = texto
